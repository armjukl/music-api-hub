...
from .bilibili_models import ResolvedAudio
...
class BilibiliClient:
    ...
    async def resolve_video(self, bvid: str, cid: int) -> ResolvedVideo:
        """Resolve a complete video, falling back to separate DASH tracks."""
        normalized_bvid = _normalise_bvid(bvid)
        if cid <= 0:
            raise ValueError("cid must be positive")

        progressive_data = await self._wbi_data(
            _PLAY_URL,
            {
                "bvid": normalized_bvid,
                "cid": cid,
        ...
        # A durl list can contain sequential segments.  Only use the direct
        # proxy path for a single complete MP4; otherwise request DASH so the
        # provider can produce one complete video.
        progressive = self._single_progressive_track(progressive_data.get("durl"))
        headers = await self.stream_headers(normalized_bvid)
        duration_ms = _to_int(progressive_data.get("timelength")) or None
        if progressive is not None:
            url, backup_urls = progressive
        ...
        dash_data = await self._wbi_data(
            _PLAY_URL,
            {
                "bvid": normalized_bvid,
                "cid": cid,
        ...
        video_track = self._select_dash_video_track(
            self._dash_video_tracks(dash_data.get("dash"))
        )
        audio_track = self._select_dash_track(
            self._dash_audio_tracks(dash_data.get("dash"))
        )
        if video_track is None or audio_track is None:
            raise BilibiliError(
                "Bilibili did not return both video and audio streams"
            )
        return ResolvedVideo(
            url=video_track.url,
            headers=headers,
            mime_type=video_track.mime_type or "video/mp4",
            backup_urls=video_track.backup_urls,
            duration_ms=_to_int(dash_data.get("timelength")) or duration_ms,
        ...
        for raw_track in raw_tracks:
            ...
            tracks.append(
                _DashVideoTrack(
                    ...
                    backup_urls=urls[1:],
                    bandwidth=max(0, _to_int(raw_track.get("bandwidth"))),
                    width=max(0, _to_int(raw_track.get("width"))),
                    height=max(0, _to_int(raw_track.get("height"))),
                    codecs=str(raw_track.get("codecs") or "").strip(),
                    mime_type=str(
                        raw_track.get("mimeType")
                        or raw_track.get("mime_type")
                        or "video/mp4"
    ...
    def _select_dash_video_track(
        ...
        if not tracks:
            return None
        # Prefer H.264 when available because it is the most broadly playable
        # codec in browser video elements, then select the highest quality.
        avc_tracks = [
            track for track in tracks if "avc" in track.codecs.lower()
        ]
        candidates = avc_tracks or list(tracks)
        return max(
            candidates,
            key=lambda track: (
                track.height,
    ...
    @staticmethod
    def _first_progressive_track(durl: Any) -> tuple[str, tuple[str, ...]] | None:
        ...
        for item in durl:
            ...
            urls = _unique_urls(
                str(item.get("url") or ""),
                item.get("backup_url") or item.get("backupUrl"),
            )
            if urls:
